            ...
            while (true){
                ...
                try { 
                    ...
                    else if (input[0] == "Order")
                    {
                        ...
                        string storeName = input[1];
                        result = controller.Report(storeName);
                    }
                ...
                catch(Exception ex)
                {
                    if (ex.Message.Contains("items for delivety"))
                    {
                        ...
                        int numForDelivery = int.Parse(messageTokens[11]);
                        string storeName = messageTokens[2];
                        writer.WriteLine($"Store: {storeName}, Items for delivery: {numForDelivery}");
                        controller.TempDelivery(storeName, numForDelivery);
